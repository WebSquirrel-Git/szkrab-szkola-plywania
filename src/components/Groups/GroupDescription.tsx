...
export const GroupDescription=({name,age,icon,description,list}:GroupDescriptionProps)=>{
    ...
    return <div className='flex flex-col xl:w-[40%] md:w-[80%] w-full gap-8 xl:items-start items-center xl:text-left text-center'>
    ...
    <p>{description}</p>
    {list.length>0&& <div className='flex flex-col gap-2.5 xl:items-start items-center sm:w-fit w-[90%]'>
        {list.map((item,i)=> <span key={i} className='flex flex-row gap-2.5 items-center sm:w-fit w-full'>
            <span className='w-[16px]  h-[16px] hidden xl:block rounded-full bg-orange'></span>
            <p className='border-b-2 border-orange sm:py-1.5 py-0.5 sm:w-fit w-full'>{item}</p>
        </span>)}
